...
public class ChessMatch {           
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof King && target.getColumn() == source.getColumn()-2) {     //se a peça foi um rei
			Position sourceT = new Position(source.getRow(), source.getColumn() - 4);   //posição da torre foi a posição do rei
			Position targetT = new Position(source.getRow(), source.getColumn() - 1);    //destino posição do rei coluna menos 1
		...
		if (p instanceof Pawn) {     
			...
			if(source.getColumn() != target.getColumn()&& capturedPiece == null) {
				...
				if(p.getColor() == Color.WHITE) {      //se a cor da peça que moveu for branca sig q a peça a ser capturada está embaixo 
